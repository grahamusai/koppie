...
export async function getCustomers(): Promise<Customer[]> {
  try {
    ...
    return rawData.map((c) => ({
      ...
      type: (c.customerType === "business" ? "Business" : "Individual") as
        | "Business"
        | "Individual",
      status: (c.status === "active"
        ...
        : c.status === "prospect"
          ? "Prospect"
          : "Inactive") as "Active" | "Prospect" | "Inactive",
      createdDate: c.createdAt.toString().split("T")[0],

      // Fill missing fields with default values
      openItems: "",
      outstanding: 0,
      lastActivity: c.updatedAt.toString().split("T")[0],
      owner: "System",
    }));
  } catch (error) {
    const err = error;
    ...
    throw new Error(
      `Failed to fetch customers: ${err.cause?.message || err.message || String(error)}`,
    );
  }
}
